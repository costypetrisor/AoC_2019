...
def solve(memory, input_vector=()):
    ...
    with open('state.dat', 'wb') as f:
        pickle.dump({
            'found_oxygen_system': found_oxygen_system,
...
def solve_part2():
    with open('state.dat', 'rb') as f:
        state = pickle.load(f)
    found_oxygen_system = state['found_oxygen_system']
    direct_connections = state['direct_connections']
    grid = state['grid']

    oxygenated = {found_oxygen_system}
    edges = set(direct_connections[found_oxygen_system])
    minute_counter = 0
    total_edges_count = len(set(direct_connections.keys()))

    while edges:
        minute_counter += 1
        print(f'At minute {minute_counter} I oxygenated {len(oxygenated)}/{total_edges_count}. I will now oxygenate {len(edges)} {edges}')
        for e in edges:
            grid[e[1]][e[0]] = Tile_print[DroidStatus.OxygenSystem.value]
        oxygenated.update(edges)
        new_edges = set()
        for edge in edges:
            dir_conns = direct_connections[edge]
            for dc in dir_conns:
                if dc not in oxygenated:
                    new_edges.add(dc)
        edges = new_edges
        # print_grid(grid)
        # time.sleep(0.2)
    print_grid(grid)
    print(f'Oxygenated {len(oxygenated)}/{total_edges_count} after {minute_counter} minutes')
