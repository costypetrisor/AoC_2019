...
def solve(grid):
    ...
    powers = []
    for i in range(25):
        powers.append(int(math.pow(2, i)))
    print(powers)
    ...
    previous_states = set()
    previous_states_grids = set()
    previous_states_grids.add(''.join(itertools.chain.from_iterable(grid)))
    ...
    while True:
        ...
        for row, line in enumerate(grid):
            for col, cell in enumerate(line):
                ...
                # if grid[y][x] == '#':
                if cell == '#':
                    # print(f'adding x={x} y={y} {x * grid_size[0] + y} power {powers[x * grid_size[0] + y]}')
                    grid_bio += powers[row * grid_size[0] + col]
        print_grid(grid)
        print(f'grid_bio={grid_bio}')
        # new_grid_flat = tuple(itertools.chain.from_iterable(new_grid))
        # new_grid_flat = ''.join(itertools.chain.from_iterable(new_grid))
        if grid_bio in previous_states:
        # if new_grid_flat in previous_states_grids:
            print(f'Found previous state: {grid_bio}')
            break
        previous_states.add(grid_bio)
        # previous_states_grids.add(new_grid_flat)
