...
def solve(memory):
    ...
    nat = collections.deque()
    nat_sent = collections.deque()
    ...
    while keep_running:
        ...
        if not sent and nat:
            ...
            computer.run_until_input_or_end(give_input=packet[1])
            computer.run_until_input_or_end(give_input=packet[2])
            while len(nat_sent) > 2:
                nat_sent.popleft()
            nat_sent.append(packet)
        if len(nat_sent) >= 2:
            if nat_sent[-1][2] == nat_sent[-2][2]:
                print(f'First nat == Y in a row: {nat_sent[-1][2]}')
                return
