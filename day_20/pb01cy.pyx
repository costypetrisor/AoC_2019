...
def solve(memory: List):
    ...
    registers = [chr(ord('A') + i) for i in range(9) ]
    print(registers)
    ...
    # for reg in registers[1:-1]:
    #     instructions.append(('NOT', reg, 'T'))
    #     instructions.append(('OR', 'T', 'J'))
    # instructions.append(('AND', registers[-1], 'J'))

    #### instructions.append(('NOT', 'A', 'J'))
    # instructions.append(('OR', 'A', 'J'))
    # for reg in registers[1: -1]:
    #     instructions.append(('AND', reg, 'J'))
    # instructions.append(('NOT', 'J', 'J'))
    # instructions.append(('AND', registers[-1], 'J'))

    print(f'nb instr={len(instructions)} instr:\n{instructions}')
    if len(instructions) > 15:
        raise Exception(f'Out of memory at most 15 instructions, we have {len(instructions)}')

    send_instructions(program, instructions)
    send_line(program, 'RUN')

    program.run_until_input_or_end()

    print('Read program input:')
